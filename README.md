...
generate backports for documentation PRs in the [Ceph repository](https://github.com/ceph/ceph).
...
* **Simplified**: Skips a lot of the rather unnecessary polish like
  configuration feature, `git` upstream reference auto-detection (hardcoded to
  `upstream`), etc.
...
Add your [GitHub personal token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens) either in the environment variable `github_token` or
in the file `$HOME/.github_token`.
...
* Edit the conflict file, make sure to search/fix/delete all git markup such as
  `>>>>>>` and `<<<<<<`.
